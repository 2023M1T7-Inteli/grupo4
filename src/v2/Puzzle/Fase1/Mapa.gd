...
func _on_Area2D_body_entered(body):
	...
	get_node("GerenciadorDeBlocos").resetList() #reset the comands list when the player lose the game or reset it
...
func _on_vitoria_body_entered(body):
	var winnerScene = load("res://winner & loser bgd/winner scene.tscn").instance()
	get_tree().current_scene.add_child(winnerScene)
	get_node("GerenciadorDeBlocos").resetList()  # change the scene to the winner scene
